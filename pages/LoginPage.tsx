...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Mocking an async login process
    setTimeout(() => {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100">
          <div className="p-10 text-center bg-indigo-600 text-white relative overflow-hidden">
             ...
             <div className="w-20 h-20 bg-white rounded-3xl mx-auto flex items-center justify-center shadow-xl mb-6 transform rotate-3 hover:rotate-0 transition-transform">
                <i className="fa-solid fa-graduation-cap text-indigo-600 text-4xl"></i>
             </div>
             <h1 className="text-3xl font-black tracking-tight uppercase">MECDA</h1>
             <p className="text-indigo-100 font-medium text-sm mt-1 opacity-80 uppercase tracking-widest">Class Management System</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="p-10 space-y-6">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-3 disabled:opacity-70"
            >
              {loading ? (
            ...
            <div className="pt-4 text-center">
               <p className="text-[10px] font-bold text-gray-400 uppercase">Secured Local Access Only</p>
            </div>
          </form>
        </div>
        <p className="text-center mt-8 text-indigo-400 text-xs font-bold uppercase tracking-widest opacity-60">
           &copy; {new Date().getFullYear()} MECDA Educational Institute
        </p>
      </div>
    </div>
  );
